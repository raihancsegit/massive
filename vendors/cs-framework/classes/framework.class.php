...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.', 'massive' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.', 'massive' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.', 'massive' ), 'updated' );
    }
  ...
  public function admin_menu() {
    ...
    if( $args['menu_type'] == 'bypass_add_submenu_page' ) {
  ...
  public function admin_page() {
    ...
    echo '<div class="cs-framework cs-option-framework">';
      ...
      echo '<h1>Massive <small>by ThemeBucket</small></h1>';
      echo '<fieldset>';
      echo ( $this->settings['ajax_save'] === true ) ? '<span id="cs-save-ajax">'. esc_html__( 'Settings saved.', 'massive' ) .'</span>' : '';
      submit_button( esc_html__( 'Save', 'massive' ), 'primary', 'save', false, array( 'data-ajax' => $this->settings['ajax_save'], 'data-save' => esc_html__( 'Saving...', 'massive' ) ) );
      submit_button( esc_html__( 'Restore', 'massive' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );
      echo '</fieldset>';
      echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options', 'massive' ) .'</a>' : '';
      ...
      echo '<em>Massive v ' . MASSIVE_VERSION . '</em> by <a href="http://themebucket.net" target="_blank">ThemeBucket</a>';
